...
\begin{document}
 ...
 \end{cases} 
 	...
 	$R_4$ is not transitive as:
 	
 	\cent{$ \exists x,y,z \In A : (x,y) \In R_4 \to (y,z) \In R_4 \to (x,z) \notin R_4 $}
 	
 	That says: \textit{"There exist an element of $A$ such that, if there exists a relation from $x$ to $y$, and if then, there exists a relation from $y$ to $z$", then there do not exists a relation from $x$ to $z$}.
 	\pagebreak
 ...
 $R$ is not symmetric, as we for a particular element $x,y$ of $R$, can't find a particular element of $R$, such that $x < y$.\\
 
 $R$ is transitive, as we can find some particular elements of $R$ such that, for any element $x$, $y$, and $z$ of $R$, if $x \geq y$, and $y \geq z$, then $x \geq z$. That means:
 
 \cent{$\forall x,y,z \In R \exists (a,b) \In \doubleR^2 : ((y,z) \In R \to (x,y) \In R \to (x,z) \In R) \wedge x \geq y \geq z $}
\end{document}
