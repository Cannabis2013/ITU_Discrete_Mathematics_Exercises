...
\newcommand{\theorem}[1]{\textit{theorem #1}}
...
\begin{document}
 ...
 \end{cases} 
	...
	\LetterEnumeration{
 		\MyItem{In how many ways can a grop of seven be assembled?}
 		
 		In how many ways can we select $r$ elements out of a set of $n$ elements? We can count all the possibilities by the following formula:
 		
 		\cent{$\binom{n}{r} = \dfrac{n!}{r!(n-r)!}$}
 		
 		By the same logic, we can calculate how many ways we can select $7$ programers out of a group of 13 programmers:
  		\mAlign{
 			\binom{13}{7} &= \dfrac{13!}{7!(13-7)!} = \dfrac{13!}{7!6!} = \dfrac{13 \cdot 12 \cdot11 \cdot 10 \cdot 9 \cdot 8}{6 \cdot 5 \cdot 4 \cdot 3 \cdot 2} \\
 			&= \dfrac{13 \cdot 2 \cdot 6 \cdot11 \cdot 2 \cdot 5 \cdot 3 \cdot 3 \cdot 2 \cdot 4}{6 \cdot 5 \cdot 4 \cdot 3 \cdot 2} \\
 			&= 13  \cdot 11   \cdot 3 \cdot 4 = 13 \cdot 12  \cdot 11 = 156  \cdot 11 = 1560 + 156 \\
 			&= 1716
 		}
	 	We can assemble a team of seven programmers out of a group of thirteen programmer in 1716 different ways.\\
	 
	 	\MyItem{Consider this composition: Seven women and six men.}
	 	
	 	\subitem{- How many teams of four women and three men can be assembled?}
	 	\subitem{- How many teams of 7 comprise at least one man can be assembled?}
	 	\subitem{- How many teams comprise at most three women can be assembled?\\}
	 	
	 	Let's first count the ways we can select a group of four women out of a total of seven women:
	 	
	 	\mAlign{
	 		\binom{7}{4} &= \dfrac{7!}{4!(7-4)!} = \dfrac{7 \cdot 6 \cdot  5 }{3 \cdot  2} \\
	 		&=  \dfrac{7 \cdot 3 \cdot 2 \cdot   5 }{3 \cdot  2} =  7  \cdot   5 = 35
	 	}
	 	
	 	Then we count the ways we can select a group of three men out of a total of six men:
	 	
	 	\mAlign{
	 		\binom{6}{3} &= \dfrac{6!}{3!(6-3)!} = \dfrac{6 \cdot  5 \cdot 4 }{3 \cdot  2} \\
	 		&=  \dfrac{3 \cdot 2 \cdot   5 \cdot 4 }{3 \cdot  2} =   5 \cdot 4 \\
	 		&=  20
	 	}
	 
	 	Then we just have to determine the product of the number of ways we can select four women and the number of ways we can select three men:
